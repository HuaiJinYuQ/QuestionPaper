...
export default {
  data() {
    return {
      ...
      questionDivScroll: 0 //记录当前滚动位置
    };
  },
  methods: {
    //初始化问卷所有问题
    init(sn, title, desc, xsWidows) {
      this.sn = sn;
      this.title = title;
      this.desc = desc;
      this.xsWidows = xsWidows;
      if (xsWidows) {
        this.dialogWidth = 80;
      }
    ...
    getQuestionList() {
      ...
      this.$axios.get(`/questions/${this.sn}`).then(data => {
        ...
        this.detail.forEach(t => {
          ...
          t.questionType = t.questionType.toString();
        });

        this.$emit("setDivScroll");
        this.loading = false;
      });
    },
    //点击添加问题按钮
    ...
    alertQuestionSourt(alert, item) {
      this.loading = true;
      this.$axios
        .post(`/questions/sortOrder/${alert}/${item.paperSN}/${item.sn}`)
        .then(res => {
          this.getQuestionList();
          this.loading = false;
        });
